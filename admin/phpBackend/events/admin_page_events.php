...
<section class="sectionimage">
    <div class="overlay-text">
        ...
        <form id="add-event-form">
            ...
            <input type="text" id="event-name" name="event_name" oninput="this.value = this.value.toUpperCase();" class="form-center"><br><br>
            <input type="submit" value="Add Event" class="eventSubmit">
        </form>
        </div>
    </div>
</section>
...
<script>
    ...
    if (event.target.classList.contains('update-button')) {
        ...
        fetch(`admin_update_events.php?id=${driverId}&active=${isChecked ? 1 : 0}`, {
            method: 'GET'
        })
        .then(response => {
            if (!response.ok) {
